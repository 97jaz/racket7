...
(define ($intmap-ref et t h key def)
  (cond
   [(Br? t)
    (if (fx<= h (Br-prefix t)) ;(zero-bit? h (Br-mask t))
   ...
   [(Br? a)
    (and
     ...
     (let ([p1 (Br-prefix a)]
           ...
           [m2 (Br-mask b)])
       (cond
        ;; PROBLEM?: if hash codes can be negative, we're going to have to
        ;; do something else. We'd really like an unsigned comparison here.
        [(fx> m1 m2) #f]
        [(fx> m2 m1)
